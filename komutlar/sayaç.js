...
exports.run = async (client, message, args) => {
  ...
  if (!mentionedChannel && args[0] !== "sıfırla")
    return message.channel.send(s1);
  ...
  if (args[0] <= message.guild.memberCount) {
    const embed = new Discord.RichEmbed()
      .setDescription(
        `Lütfen, [${message.guild.memberCount}] rakamlı sayıdan daha yüksek bir değer belirtiniz!`
      )
      .setColor("RANDOM")
      .setTimestamp();
    message.channel.send({ embed });
  ...
  const embed = new Discord.RichEmbed()
    .setDescription(
      `Sayaç, başarılı bir şekilde \`${
        args[0]
      }\` olarak ayarlandı, sayaç kanalı ise ${mentionedChannel} olarak ayarlandı!`
    )
